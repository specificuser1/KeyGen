        ...
        for category in ["BP", "HK"]:
            ...
            if os.path.exists(folder_path):
                for file in os.listdir(folder_path):
                    if file.endswith('.txt') and not file.endswith('.disabled'):
                ...
                if "KEY MANAGER PANEL" in embed.title or "Panel" in embed.title:
                    ...
                    if self.bot.db is not None:
                        try:
                            await self.bot.db.panels.update_one(
                                {"guild_id": interaction.guild.id, "channel_id": interaction.channel.id},
                                {"$set": {"message_id": message.id, "guild_id": interaction.guild.id, "channel_id": interaction.channel.id}},
                                upsert=True
                            )
                        except:
                            pass
...
class PersistentPanelView(discord.ui.View):
    def __init__(self, bot):
        ...
        select_menu = discord.ui.Select(
            custom_id="panel_main_select",
            placeholder="🔧 Select an action...",
            options=select_options,
            min_values=1,
            max_values=1
        )
        select_menu.callback = self.panel_select_callback
        ...
        for category in ["BP", "HK"]:
            ...
            if os.path.exists(folder_path):
                for file in sorted(os.listdir(folder_path)):
                    if file.endswith('.txt') or file.endswith('.txt.disabled'):
                        is_disabled = file.endswith('.disabled')
                        actual_file = file.replace('.disabled', '')
                        duration = actual_file.replace('.txt', '')
                        duration_name = self.bot.config['duration_names'].get(duration, duration)
    ...
    async def remove_duration_menu(self, interaction: discord.Interaction, permanent: bool):
        ...
        if permanent:
            ...
            title = "⏸️ Temporarily Remove Duration"
            description = "Select a duration to **toggle disable/enable**.\n💡 Disabled durations can be restored."
        
        embed = discord.Embed(title=title, description=description, color=discord.Color.from_str(self.bot.config['embed_color']))
        ...
        for duration, name in self.bot.config['duration_names'].items():
            ...
            for category in ["BP", "HK"]:
                if os.path.exists(f"./{category}/{duration}.txt.disabled"):
                    is_disabled = True
                    break
            status = " ⏸️ (Disabled)" if is_disabled else " ✅"
    ...
    async def change_role_prompt(self, interaction: discord.Interaction):
        embed = discord.Embed(title="👥 Change Allowed Role", description="Click the button below to change the allowed role ID", color=discord.Color.from_str(self.bot.config['embed_color']))
        
        current_roles = ", ".join(self.bot.config.get('allowed_role_ids', []))
        embed.add_field(name="Current Allowed Role IDs", value=f"```{current_roles if current_roles else 'None'}```", inline=False)
...
class AddKeysView(discord.ui.View):
    def __init__(self, bot, previous_view):
        ...
        select_menu = discord.ui.Select(
            custom_id="addkeys_category_select",
        ...
        select_menu.callback = self.category_callback
        self.add_item(select_menu)
...
class DurationSelectView(discord.ui.View):
    def __init__(self, bot, category, previous_view):
        ...
        for duration, name in bot.config['duration_names'].items():
            is_disabled = os.path.exists(f"./{category}/{duration}.txt.disabled")
            if not is_disabled:
                options.append(discord.SelectOption(label=name, value=duration, description=f"Add keys for {name}", emoji="⏰"))
        
        if options:
            select_menu = discord.ui.Select(
                custom_id="duration_select_menu",
                placeholder="Select duration...",
                options=options[:25]
            )
            select_menu.callback = self.duration_callback
            self.add_item(select_menu)
        else:
            select_menu = discord.ui.Select(
                custom_id="duration_select_disabled",
    ...
    @discord.ui.button(label="↩ Back", style=discord.ButtonStyle.gray, custom_id="duration_back_btn", row=2)
...
class RemoveDurationView(discord.ui.View):
    def __init__(self, bot, previous_view, permanent):
        ...
        options = []
        for duration, name in bot.config['duration_names'].items():
            options.append(discord.SelectOption(label=name, value=duration, emoji="🗑️" if permanent else "⏸️"))
        
        if options:
            select_menu = discord.ui.Select(
                custom_id="remove_duration_select",
                placeholder=f"Select duration to {'delete' if permanent else 'toggle'}...",
                options=options[:25]
            )
            select_menu.callback = self.remove_callback
            self.add_item(select_menu)
    ...
    async def toggle_duration(self, interaction, duration):
        ...
        embed = discord.Embed(
            ...
            description=f"**{self.bot.config['duration_names'][duration]}** is now {status}",
            color=discord.Color.green(),
            timestamp=datetime.now()
        )
        embed.set_footer(text=f"Powered by {self.bot.config['powered_by']} | Dev: {self.bot.config['dev_name']}")
...
class ConfirmRemoveView(discord.ui.View):
    ...
    @discord.ui.button(label="✅ Confirm Delete", style=discord.ButtonStyle.danger, custom_id="confirm_delete_btn")
    async def confirm_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        deleted_name = config['duration_names'].get(self.duration, self.duration)
        ...
        embed = discord.Embed(
            ...
            description=f"**{deleted_name}** has been permanently deleted.",
            color=discord.Color.green(),
            timestamp=datetime.now()
        )
        embed.set_footer(text=f"Powered by {self.bot.config['powered_by']} | Dev: {self.bot.config['dev_name']}")
    ...
    @discord.ui.button(label="❌ Cancel", style=discord.ButtonStyle.gray, custom_id="cancel_delete_btn")
...
class NewDurationModal(discord.ui.Modal, title="Create New Duration"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if code in config['duration_names']:
            embed = discord.Embed(title="❌ Error", description=f"Duration code `{code}` already exists!", color=discord.Color.red())
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
...
class AddKeysModal(discord.ui.Modal, title="➕ Add Keys"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="🔑 Keys", value=key_preview, inline=False)
...
class ChangeRoleModal(discord.ui.Modal, title="👥 Change Allowed Role ID"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            embed = discord.Embed(title="❌ Invalid Role ID", description="Please enter a valid numeric Discord role ID!", color=discord.Color.red())
            embed.set_footer(text=f"Powered by {self.bot.config['powered_by']} | Dev: {self.bot.config['dev_name']}")
            await interaction.response.send_message(embed=embed, ephemeral=True)
